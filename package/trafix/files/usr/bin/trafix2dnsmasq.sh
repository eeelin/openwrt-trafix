...
# Name:        trafix2dnsmasq.sh
# Desription:  A shell script which convert trafix data into dnsmasq rules.
...
Usage: sh trafix2dnsmasq.sh [options] -o FILE
Valid options are:
    -d, --dns <dns_ip>
...
process(){
    ...
    TMP_DIR=`mktemp -d /tmp/trafix2dnsmasq.XXXXXX`
    BASE64_FILE="$TMP_DIR/base64.txt"
    SOURCE_FILE="$TMP_DIR/upstream-rules.txt"
    DOMAIN_TEMP_FILE="$TMP_DIR/trafix2domain.tmp"
    DOMAIN_FILE="$TMP_DIR/trafix2domain.txt"
    CONF_TMP_FILE="$TMP_DIR/trafix.conf.tmp"
    OUT_TMP_FILE="$TMP_DIR/trafix.out.tmp"

    # Fetch the upstream rule source and decode it into plain text
    printf 'Fetching upstream rules... '
    if [ $USE_WGET = 0 ]; then
        echo "curl..."
    ...
    if [ $? != 0 ]; then
        _red '\nFailed to fetch upstream rules. Please check your Internet connection, and check TLS support for curl/wget.\n'
        clean_and_exit 2
    fi
    $BASE64_DECODE $BASE64_FILE > $SOURCE_FILE || ( _red 'Failed to decode upstream rules. Quit.\n'; clean_and_exit 2 )
    _green 'Done.\n\n'
    ...
    printf 'Converting rules to ' && _green $OUT_TYPE && printf ' ...\n' 
    _yellow '\nWARNING:\nThe following lines in GfwList contain regex, and might be ignored:\n\n'
    cat $SOURCE_FILE | grep -n '^/.*$'
    ...
    grep -vE $IGNORE_PATTERN $SOURCE_FILE | $SED_ERES $HEAD_FILTER_PATTERN | $SED_ERES $TAIL_FILTER_PATTERN | grep -E $DOMAIN_PATTERN | $SED_ERES $HANDLE_WILDCARD_PATTERN > $DOMAIN_TEMP_FILE
...
ipset=/\1/'$IPSET_NAME'#g' > $CONF_TMP_FILE
        ...
        echo '# dnsmasq rules generated by trafix' > $OUT_TMP_FILE
